...
pub fn get_function_spec_strings() -> Result<Vec<String>, ParseError> {
    ...
    let path = std::env::current_dir().or(Err(ParseError::NoESSSFile))?.into_string().or(Err(ParseError::NoESSSFile))? + "/results_ai_analysis.txt"; 
    println!("AI Results File should be at {}", path);
...
pub fn parse_ai() -> Option<Vec<FunctionErrorSpec>> {

    eprintln!("AI parser active!");
...
pub fn print_ai_statistics(results: &Option<Vec<FunctionErrorSpec>>) {
